...
void PingOut::start_multi_impulses_scheduler(int idx, int pfx, int num_impulses) {
	...
	scheduled_idx = idx/samples_per_half_period;
    multiple_impulses_schedule_enable = true;
}
...
void PingOut::update() {
    ...
    if (PingOut::time_to_schedule_period){
        ...
        //multiple impulses scheduling:
        if (multiple_impulses_schedule_enable) {
            scheduled_pfx = PingOut::cur_out_pfx;
            PingOut::time_to_schedule_period = false;
        }
    }
...
void first_half_written_callback(DMA_HandleTypeDef *hdma) {
    ...
    //first impulse of multiple impulses scheduling
    if (PingOut::cur_out_pfx > PingOut::first_impulse_pfx &&
    		PingOut::cur_out_pfx < PingOut::last_impulse_pfx){
        PingOut::time_to_schedule_period = true;
    }
    //second impulse of multiple impulses scheduling.
    if (PingOut::cur_out_pfx == PingOut::last_impulse_pfx){
    	PingOut::first_impulse_pfx = -1;
    	PingOut::last_impulse_pfx = -1;
    }
