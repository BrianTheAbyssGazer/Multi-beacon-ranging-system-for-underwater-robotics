...
int main(void)
{
  ...
  while (1)
  {
    ...
    //tof_master_main(&hadc4, &htim3, &huart2, &hopamp2, &hopamp3, &hdma_tim2_up, &htim2);
	#endif
    const uint8_t con[5]={0,0,0,0,7};
    HAL_UART_Transmit_IT(&huart2, con, 5);
  }
  /* USER CODE END 3 */
}
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : ping_drive_Pin PC8 LED_out_Pin PC10
                           PC11 */
  GPIO_InitStruct.Pin = ping_drive_Pin|GPIO_PIN_8|LED_out_Pin|GPIO_PIN_10
                          |GPIO_PIN_11;
