...
void IndexInfoTX :: stream_adc(int adc_val) {
	uint8_t bytes[4];
	bytes[0] = static_cast<uint8_t>(adc_val >> 0);
	bytes[1] = static_cast<uint8_t>(adc_val >> 8);
	bytes[2] = static_cast<uint8_t>(adc_val >> 16);
	bytes[3] = static_cast<uint8_t>(adc_val >> 24);
