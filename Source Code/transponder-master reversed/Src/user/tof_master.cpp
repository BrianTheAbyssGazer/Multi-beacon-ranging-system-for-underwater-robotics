		...
		if(tmsp.pfx==-1){
			...
			if(waiting_time==14){
				...
				if(data==id){
					...
					waiting_time = 0;
					echo=true;
					max_peak_detector.send_data2computer(2,tmsp.pfx,uint16_t(id));
				}
				else{
					max_peak_detector.send_data2computer(1,tmsp.pfx,uint16_t(id));
					echo=false;
				}
				waiting_time = 80;
				for(int i=0;i<15;i++) signal[i]=0;
				...
				max_peak_detector.send_data2computer(0,tmsp.pfx,waiting_time);
			}
		}
		ping_out.update();
    }
